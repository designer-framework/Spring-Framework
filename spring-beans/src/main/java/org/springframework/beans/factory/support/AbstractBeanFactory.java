...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
			...
			try {
				...
				if (dependsOn != null) {
					for (String dep : dependsOn) {
						...
						//在专门存放被依赖的bean容器查找依赖的bean是否存在, 不存在则创建
						/**
						 * 调用该方法创建
				...
				if (mbd.isSingleton()) {
					/**
					 * @see ObjectFactory 可以理解成Supplier, 通过
					 * @see #createBean(String, RootBeanDefinition, Object[]) 生成 bean
					 */
	...
	protected Object getObjectForBeanInstance(
		...
		//当前注入的bean实现了FactoryBean, 则会调用自身的getObject()方法来获取实例
		if (object == null) {
