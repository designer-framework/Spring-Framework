...
class ConfigurationClassParser {
	...
	protected final SourceClass doProcessConfigurationClass(ConfigurationClass configClass, SourceClass sourceClass)
		...
		/**
		 * 该注解会扫描指定包下带@Component注解的类文件,并放到以下缓存中
		 * @see this#configurationClasses
		 * 解析完所有类后, 统一注入至容器. 注:  带有@Bean注解的方法都会被视为一个bean
		 */
		...
		//处理标记了@Import注解的类, 该注解类似@Configuration注解
		/**
		 * BEGIN
		 * 处理优先顺序, 若实现了Aware接口,会对接口中的方法进行类型判断并强转调用:
		 * 1.实现了ImportSelector接口
		 * 	（1）如果实现了DeferredImportSelector接口那么会对其进行包装,将会在后续代码中被延迟加载
		 * 	 (2)如果仅仅实现了ImportSelector接接口则直接进行反射调用并实例化, 如果被实例化的类也有@Import注解, 则进行递归调用，直到没有该注解出现
		 * 2.实现了ImportBeanDefinitionRegistrar接口的功能类似 BeanDefinitionRegistryBeanPostprocessor接口
		 * 3.若没有实现上述两个接口. 会被当成类似@Configuration配置类进行解析, 与 1 行为类似，
		 * 	同时它还会去读取当前元数据中的@ComponentScan注解,若存在则在该基础包对导入的类进行扫描
		 * 	END
		 *  *****上述产生的代价在的对象,会在该代码段的末尾被处理*****
		 */
